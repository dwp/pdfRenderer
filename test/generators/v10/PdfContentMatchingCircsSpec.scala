package generators.v10

import generators.PdfContentMatchingSpec
import play.api.test.WithApplication
import test_data.v10.circs.XMLCircsData
...
class PdfContentMatchingCircsSpec extends PdfContentMatchingSpec{
  ...
  "PdfCircsContentMatching" should {
    ...
    "extract PDF for functionalTestCase33 and match contents" in new WithApplication{
      ...
      val source = getClass.getResource(s"/${version}/circs/c3_functional33_circs.xml")
      val textCaseXml = XML.load(source)
      testContentMatches(pdfFileLocation, textCaseXml, XMLCircsData.startedAndOngoingEmployment, foundMustBeTrue)
    }

    // Employment : Started and Ongoing employment : Pension, PaidToDoThings, Costs - yes
    "extract PDF for functionalTestCase34 and match contents" in new WithApplication{
      val pdfFileLocation = s"${version}_functionalTestCase34_circs_contentTestPDF.pdf"
      val source = getClass.getResource(s"/${version}/circs/c3_functional34_circs.xml")
      val textCaseXml = XML.load(source)
      testContentMatches(pdfFileLocation, textCaseXml, XMLCircsData.startedAndOngoingEmployment, foundMustBeTrue)
    }
