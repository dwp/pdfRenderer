...
public class PdfRendererServiceTest {
    ...
    public void acceptXMLAndReturnBADREQUESTErrorIfUnknownXMLType() throws Exception {
        final byte[] status = pdfRendererService.generatePdf("<Invalid>type</Invalid>");
    ...
    public void notAcceptNonXMLRequest() throws Exception {
        final byte[] status = pdfRendererService.generatePdf("Hello");
        assertThat(new String(status, "UTF-8"), is("<Error>Failed to render XML for transactionId: []</Error>"));
    }

    @Test
    public void returnInternalErrorCodeIfCouldNotGeneratePDF() throws Exception {
        final PdfGenerator reportGenerator = mock(PdfGenerator.class);
        final Counters counters = mock(Counters.class);
        final PdfRendererService pdfRendererServiceMock = new PdfRendererService(reportGenerator, counters);
        final byte[] status = pdfRendererServiceMock.generatePdf(ClaimBuilder.goodClaim());
    ...
    public void acceptValidXMLAndGenerateAPDF() throws Exception {
        final byte[] status = pdfRendererService.generatePdf(ClaimBuilder.goodClaim());
